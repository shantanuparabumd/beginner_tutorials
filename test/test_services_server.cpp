...
void handle_add_two_ints_noreqid(
  const std::shared_ptr<ModifyString::Request> request,
  std::shared_ptr<ModifyString::Response> response) {
  response->c = request->a + request->b;
}
/**
 * @brief Hnadler for Service Request
 * 
...
public std::enable_shared_from_this<DeferedCbServiceWrapper> {
 public:
  ...
  void create_service(rclcpp::Node & node) {
    impl_ = node.create_service<ModifyString>(
      ...
      [this](
        ...
        if (handle_defered_response_thread_.joinable()) {
          throw std::runtime_error{
            "expected the callback to be called only once"};
        }
  ...
  /**
   * @brief Destroy the Defered Cb Service Wrapper object
   * 
   */
 ...
 private:
  std::shared_ptr<rclcpp::Service<ModifyString>> impl_;
...
int main(int argc, char ** argv) {
  ...
  auto service_noreqid = node->create_service<ModifyString>(
    "add_two_ints_noreqid", handle_add_two_ints_noreqid);

  auto service_reqid = node->create_service<ModifyString>(
    "add_two_ints_reqid", handle_add_two_ints_reqid);

  auto service_return_req = node->create_service<ModifyString>(
  ...
  auto derefed_cb_with_handle = node->create_service<ModifyString>(
    "add_two_ints_defered_cb_with_handle",
    [node, &timer](
      const std::shared_ptr<rclcpp::Service<ModifyString>> handle,
      const std::shared_ptr<rmw_request_id_t> request_header,
      const std::shared_ptr<ModifyString::Request> request) {
      // We defer handling the response in another callback,
      // for example a timer.
